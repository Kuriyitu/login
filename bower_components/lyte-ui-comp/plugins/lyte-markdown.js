...
( function() {
	...
	*/
	function buildConfig( totalConfiguration ) {
		var attributes = [ 
						'bold', 'italics', 'strike', 'underline', 'blockquote', 'code',
						'para',  
						'hyperlink', 'image', 
	...
	function _rDP( list, totalConfiguration ) {
		...
		var buildTable = function() {
			...
			isProperSyntax = consumeHeaderFooter( tableHeader.row_count );
			...
			isProperSyntax = buildTableBody( tableHeader.row_count );
		...
		var getTableRow = function( row_count ) {
			var row = document.createElement( 'tr' ), cell,
			mem = capture(), currentRowCount = 0;
			...
			if( currentRowCount < row_count ) {
				addRemainingColumns( row, currentRowCount, row_count );
			}
		...
		var consumeHeaderFooter = function( row_count ) {
			var rowCount = 0, hasNonTrailingCharacters;
		...
		var getTableHeader = function() {
			...
			row_count, head, hasNonTrailingCharacters;
		...
		var getLink = function() {
			...
			if( !cs.callStackContains( 'link' ) ) {		
				...
				if( list.eat().get( 'type' ) !== 'special]' || text.trim().length === 0 ) {
					return false;
				}
		...
		var getHighlight = function() {
			...
			if( cs.isEmpty() ) {
				...
				if( ret.trim().length !== 0 && isHighlight() ) {
					...
					code.appendChild( document.createTextNode( ret ) );
		...
		var isTextContentWhenTable = function( token ) {
			return isText( token ) || token.get( 'type' ) === 'special.' || token.get( 'type' ) === 'special"';
		}

		var isTextContentWhenNormal = function( token ) {
			return isText( token ) || token.get( 'type' ) === 'special.' || token.get( 'type' ) === 'special|' || token.get( 'type' ) === 'special"';
		}
