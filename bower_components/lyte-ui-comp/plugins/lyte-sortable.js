	...
	if(lyteDomObj){			
		...
		lyteDomObj.prototype.sortable = function(object) {
			...
			if(element._sortableParentData){
				...
				data.preventDefault = data.preventDefault == undefined ? { "mousedown" : true, "mousemove" : true } : data.preventDefault;
				...
				if(_sortableElem && checkForSortable(_sortableElem._sortableChildData || _sortableElem.parentElement._sortableParentData, event.target) && checkForItems(_sortableElem._sortableChildData || _sortableElem.parentElement._sortableParentData, event.target)){
					...
					if(returnVal){
						...
						if(event.type == "mousedown"){
							...
							document.addEventListener('touchmove',data._parentElem.__mouseMoveEvent, true);
				...
				if(_sortableElem){
					...
					if(data && data._isDown){
						...
						if(!data._placedPlaceholder){
							...
							var width = /*$L(_sortableElem).outerWidth() divOffset.width*/ _sortableElem.offsetWidth;
							var height = /*$L(_sortableElem).outerHeight() divOffset.height*/ _sortableElem.offsetHeight;
							var cellSpacing = 0;
							...
							if(_lyteUiUtils.getRTL() && detectBrowser() == "chrome" && data._isRelative && data.scrollDivX.contains(_div)){
											...
											if($L(_elemBelow).hasClass('sortable-parent') && checkParentDroppable(_elemBelow,_sortableElem.parentElement,_sortableElem,data.connectedWith,data.containmentDimensions,_mousePosition) && checkForIntersect(_elemBelow,_mousePosition) && checkForBetween(_elemBelow,_mousePosition,_sortableElem/*,isRelativeY,scrollDiv*/)){
												...
												if(data.tolerance == "pointer"){
													if(data.orientation === "horizontal"){
														...
														if(event.clientY < data._prevTop && (droppablePlace.previousElementSibling != _placeholder /*!_placeholder.nextElementSibling || (_placeholder.nextElementSibling && _placeholder.nextElementSibling != droppablePlace)*/)){	//!placeholder.nextElementSibling to check if placeholder is the last element and the draggable element's top goes above the last element's top
															// console.log("calling for droppable ele insertBefore => ",droppablePlace.textContent);
															_lyteUiUtils.insertBefore(droppablePlace,_placeholder);
															callPlaceholder(data, _elemBelow, droppablePlace);
														}
														else if(event.clientY > data._prevTop && (droppablePlace.nextElementSibling != _placeholder /*!_placeholder.previousElementSibling || (_placeholder.previousElementSibling && _placeholder.previousElementSibling != droppablePlace)*/)){
												...
												if(data.tolerance == "intersect"){
													if(data.orientation === "horizontal"){
														...
														if(divOffset.top < (droppableOffset.top) && ( droppablePlace.previousElementSibling != _placeholder /*(_placeholder.nextElementSibling && _placeholder.nextElementSibling != droppablePlace) || (_placeholder.parentElement != droppablePlace.parentElement)*/)){
															// console.log("calling for droppable ele insertBefore => ",droppablePlace.textContent);
															_lyteUiUtils.insertBefore(droppablePlace,_placeholder);
															callPlaceholder(data, _elemBelow, droppablePlace);
														}
														else if(divOffset.bottom > (droppableOffset.bottom) && ( droppablePlace.nextElementSibling != _placeholder /*(_placeholder.previousElementSibling && _placeholder.previousElementSibling != droppablePlace) || (_placeholder.parentElement != droppablePlace.parentElement)*/)){
															// console.log("calling for droppable ele insertAfter => ",droppablePlace.textContent);
															_lyteUiUtils.insertAfter(droppablePlace,_placeholder);
															callPlaceholder(data, _elemBelow, droppablePlace);
														}
			...
			var mouseUpEvent = function(event){
				...
				if(_sortableElem){
					...
					if(data._isDown){
						...
						_placeholderParent = _placeholder.parentElement;
						if(data._isMoved){
							data._isMoved = false;
			...
			var appendPlaceholderAt = function(data, pos){
				...
				if(pos === "first"){
					...
					for(var i = 0; i < children.length; i++){
						if(!data.preventDropAtStart || isNotRestricted(data, children[i])){
					...
					for(var i = children.length - 1; i >= 0; i--){
						if(!data.preventDropAtEnd || isNotRestricted(data, children[i])){
			...
			var onDragStart = function(data){
				data.onDragStart(data._div,data._source);
			}
			...
			var onDrag = function(data,event){
				...
				data.onDrag(data._div,data._returnElemBelow,event,placeholder);
			}
			...
			var fixWidth = function(element){
				...
				for(var i = 0; i<childrens.length; i++){
					if(childrens[i].tagName.toLowerCase() == "td" || childrens[i].tagName.toLowerCase() == "lyte-td"){
						width[ i ] = childrens[i].style.width || ( childrens[i].offsetWidth + 'px' ) ;
					}
				}
				for(var i = 0; i<childrens.length; i++){
					if(childrens[i].tagName.toLowerCase() == "td" || childrens[i].tagName.toLowerCase() == "lyte-td"){
			...
			var isHelper = function(elem){
				return elem.classList.contains('sortable-helper');
			};
