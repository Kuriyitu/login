		...
		managerDD = {
			...
			_drag : function(event){
				...
				for(var i=0; i< droppables.length ; i++){
					...
					if(!data.disabled && !(draggable.isEqualNode(droppables[i])) && managerDD._checkAcceptable(draggable,data.accept) && managerDD._isNotRejected(draggable,data.reject)){
			...
			deleteDroppable : function(element,currentWin,direction){
				if(direction == "up"){
					while(currentWin.parent){
						currentWin = currentWin.parent;
						if(currentWin.managerDD && currentWin.managerDD._droppables.indexOf(element) !== -1){
							currentWin.managerDD._droppables.splice(currentWin.managerDD._droppables.indexOf(element),1);
						}
				...
				if(direction == "down"){
					...
					if(iframes){
						for(var i = 0; i<iframes.length ; i++){
							...
							if(win.managerDD && win.managerDD._droppables.indexOf(element) !== -1){
								win.managerDD._droppables.splice(win.managerDD._droppables.indexOf(element),1);
								win.managerDD.deleteDroppable(element, win, "down");
							}
						}
					}
				}
