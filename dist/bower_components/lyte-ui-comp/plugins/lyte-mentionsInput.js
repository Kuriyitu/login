	...
	if(lyteDomObj){	
		      ...
		      var attrs = [ 'paddingTop', 'paddingRight', 'paddingBottom', 'paddingLeft', 'marginTop', 'marginRight', 'marginBottom', 'marginLeft', 'fontSize', 'lineHeight', 'fontFamily', 'width', 'fontWeight', 'border-top-width', 'border-right-width', 'border-bottom-width', 'border-left-width', 'borderTopStyle', 'borderTopColor', 'borderRightStyle', 'borderRightColor', 'borderBottomStyle', 'borderBottomColor', 'borderLeftStyle', 'borderLeftColor', 'box-sizing', '-moz-box-sizing', '-webkit-box-sizing'];
		        ...
		        var updateTextarea = function(forced) {
		          ...
		          if (forced || textareaContent + '&nbsp;' !== cloneContent) {
		            ...
		            if (Math.abs(cloneTextarea.outerHeight() + lineHeight - mITextarea.outerHeight()) >= 3 || textareaContent === "") {
		...
		lyteDomObj.prototype.mentionsInput = function(object) {
		   ...
		   //    textarea.dispatchEvent(evt);
			...
			var checkForDeletedUser = function(textarea){
				var collection = textarea._mIData.lyteMICollection;
				var value = textarea.value;
				var currentMessage = value.substring(0,textarea.selectionEnd);
				var returnVal = false;
				var start,end;
				for(var i = collection.length - 1; i >= 0; i--) {
			        if((collection[i].position.start > textarea.selectionStart && collection[i].position.start <= textarea.selectionEnd) || (collection[i].position.end >= textarea.selectionStart && collection[i].position.end <= textarea.selectionEnd)){
			        	returnVal = true;
			        	if(start == undefined || collection[i].position.start < start){
			        		start = collection[i].position.start;
			        	}
			        	if(end == undefined || collection[i].position.end > end){
			        		end = collection[i].position.end;
			        	}
			        }
			    };
			    if(returnVal){
			    	start = start < textarea.selectionStart ? start : textarea.selectionStart;
			    	end = end > textarea.selectionEnd ? end : textarea.selectionEnd;
			...
			var updateCollectionPosition = function(textarea){
				var collection = textarea._mIData.lyteMICollection;
				var message = $L(textarea._mIData.element).mentionsInput("getMessage");
				collection.forEach(function(mention){
					...
					if(mention.position.start != index || mention.position.end != index+mentionObj.value.length){
