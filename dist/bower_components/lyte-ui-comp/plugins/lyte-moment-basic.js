		...
		if( lyteDomObj ){
			...
			function convertFormat( arg ) {
				...
				for( var i = 0; i < parseVal.length; i++ ) {
					...
					switch( cur.type ) {
						case "date" : {
							arg = arg.replace( cur.val + lm, crctLength( ( cur.year ? totdate( obj.month, isLeap( obj.year ), obj.date ) : obj.date ), cur.len, cur.suff ) )
						}
						break;
						case "month" : {
							if( cur.str ) {
								arg = arg.replace( cur.val + lm, cur.array[ obj.month ] );
							} else {
								arg = arg.replace( cur.val + lm, crctLength( obj.month + 1, cur.len, cur.suff ) );
							}
						}
						break;
						case "year" : {
						...
						break;
						case "quarter" : {
							arg = arg.replace( cur.val + lm, crctLength( Math.ceil( ( obj.month + 1 ) / 3 ), null , cur.suff ) )
						}
						break;
						case 'longdate' : {
							if( cur.str ) {
								arg = arg.replace( cur.val + lm, cur.array[ obj.day ]);
							} else {
								arg = arg.replace( cur.val + lm, crctLength( obj.day, null , cur.suff ) )
							}
						}
						break;
						case 'hour' : {
						...
						break;
						case 'minute' : {
							arg = arg.replace( cur.val + lm, crctLength( date.getMinutes(), cur.len ) )
						}
						break;
						case 'second' : {
							arg = arg.replace( cur.val + lm, crctLength( date.getSeconds(), cur.len ) )
						}
						break;
						case 'millisecond' : {
							arg = arg.replace( cur.val + lm, crctLength( date.getMilliseconds(), 3 ) ).replace( /\\S+/, '' )
						}
						break;
						case 'timezone' : {
							var val = '';
				...
				if( isWeek.flag ) {
					...
					if( isWeek.week ){
						for( var j = 0; j < isWeek.week.length; j++ ){
							arg = arg.replace( isWeek.week[ j ].val + lm, week + crctLength( ret.week, isWeek.week[ j ].len, isWeek.week[ j ].suff ) );
						}
					}
					if( isWeek.day ) {
			...
			lyteMoment.prototype = {
				...
				isSame : function( arg ){
					if( this._isValid && arg && arg._isMoment && arg._isValid ) {
